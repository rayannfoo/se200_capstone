...
import { PolicyHolderTable } from "@/components/policyHolders/policyHolder-table";

export default async function CustomersPage() {
  // Fetch customer data
  const policyHolder = await getPolicies();

  if (!Array.isArray(policyHolder)) {
    return <div>Error: {policyHolder.error}</div>;
  }
  ...
  return (
    <div className="flex h-screen">
      ...
      <div className="flex-1 bg-gray-200 p-6 ml-24">
        ...
        <PolicyHolderTable policies={policyHolder} offset={0} />
      </div>
    </div>
  );
}
